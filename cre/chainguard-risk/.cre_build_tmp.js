...
var REPORT_RISK_PARAMS = parseAbiParameters("address target, uint256 score, string vulnerability");
var buildRiskRequest = (address, riskApiUrl) => (sendRequester, _config) => {
  const req = {
...
function onHttpTrigger(runtime2, payload) {
  runtime2.log("════════════════════════════════════════════════════════");
  runtime2.log("  ChainGuard CRE — Smart Contract Risk Detection");
  runtime2.log("  Powered by Chainlink CRE + Gemini AI + Etherscan");
  runtime2.log("════════════════════════════════════════════════════════");
  ...
  runtime2.log(`[Step 1] Target contract: ${contractAddress}`);
  runtime2.log(`[Step 2] Calling ChainGuard Risk API: ${runtime2.config.riskApiUrl}`);
  const httpClient = new cre.capabilities.HTTPClient;
  const riskData = httpClient.sendRequest(runtime2, buildRiskRequest(contractAddress, runtime2.config.riskApiUrl), consensusIdenticalAggregation())(runtime2.config).result();
  runtime2.log(`[Step 3] ── Analysis Complete ──`);
  runtime2.log(`[Step 3]   Score       : ${riskData.score}/100`);
  runtime2.log(`[Step 3]   Vulnerability: ${riskData.vulnerability}`);
  runtime2.log(`[Step 3]   Value USD   : $${riskData.value_usd.toFixed(2)}`);
  runtime2.log(`[Step 3]   Risk Level : ${riskData.score >= 85 ? "\uD83D\uDD34 CRITICAL" : riskData.score >= 70 ? "\uD83D\uDFE0 HIGH" : riskData.score >= 50 ? "\uD83D\uDFE1 MEDIUM" : "\uD83D\uDFE2 LOW"}`);
  if (riskData.score >= 70) {
    runtime2.log(`[Step 4] ⚠️  DANGER DETECTED — Publishing onchain alert...`);
    ...
    runtime2.log(`[Step 4] Writing to RiskRegistry: ${evmConfig.registryAddress}`);
    ...
    if (writeResult.txStatus === TxStatus.SUCCESS) {
      ...
      runtime2.log(`[Step 5] ✅ ONCHAIN ALERT PUBLISHED!`);
      runtime2.log(`[Step 5]   TxHash  : ${txHash}`);
      runtime2.log(`[Step 5]   Registry: ${evmConfig.registryAddress}`);
      runtime2.log(`[Step 5]   Contract: ${contractAddress}`);
      runtime2.log(`[Step 5]   Score   : ${riskData.score}/100`);
      runtime2.log("════════════════════════════════════════════════════════");
      return txHash;
    }
    throw new Error(`Transaction failed with status: ${writeResult.txStatus}`);
  }
  runtime2.log(`[Step 4] ✅ Contract is safe (score: ${riskData.score}/100)`);
  runtime2.log("════════════════════════════════════════════════════════");
